  ...
  describe Api::LegacyReturnAuthorizationsController do
    ...
    context "as the order owner" do
      ...
      it "cannot see any legacy return authorizations" do
        get :index, params: { order_id: order.to_param, format: :json }
        assert_unauthorized!
      end

      it "cannot see a single legacy return authorization" do
        get :show, params: { order_id: order.to_param, :id => 1, format: :json }
        assert_unauthorized!
      end
    end
    ...
    context "as an admin" do
      ...
      it "can add an inventory unit to a legacy return authorization on the order" do
        ...
        put :add, params: { :id => legacy_return_authorization.id, variant_id: inventory_unit.variant.id, quantity: 1, order_id: order.to_param, format: :json }
        expect(response.status).to eq(200)
        expect(json_response).to have_attributes(attributes)
